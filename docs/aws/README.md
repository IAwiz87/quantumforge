...
4. Apply `live-validation-permissions-boundary.json` both as the role policy and as its permissions boundary. Keep approval on the manual live-test environment; keep the main-only janitor environment automatic so cleanup is never queued behind approval. Retain the account-ID runtime guard in both.
