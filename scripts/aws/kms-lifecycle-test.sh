...
cleanup() {
  ...
  if [[ -d "$TF_DATA_DIR" ]]; then
    for _ in 1 2 3; do
      if TF_DATA_DIR="$TF_DATA_DIR" terraform -chdir="$FIXTURE_DIR" destroy -auto-approve -input=false \
        -var="aws_region=$AWS_REGION" -var="run_id=$RUN_ID" -var="expires_at=$EXPIRES_AT" \
  ...
  if [[ -n "$key_id" ]]; then
    if aws kms describe-key --region "$AWS_REGION" --key-id "$key_id" \
      --query 'KeyMetadata.{KeyState:KeyState,DeletionDate:DeletionDate}' \
      --output json > "$EVIDENCE_DIR/key-cleanup-status.json" 2>/dev/null && \
      jq -e '.KeyState == "PendingDeletion" and (.DeletionDate | type == "string" and length > 0)' \
        "$EVIDENCE_DIR/key-cleanup-status.json" >/dev/null; then
      pending_deletion_verified=1
    fi
  fi
  rm -rf "$TMP_DIR"
  if [[ "$rc" -eq 0 && ( "$destroy_succeeded" -ne 1 || "$pending_deletion_verified" -ne 1 ) ]]; then
    echo "KMS cleanup did not prove Terraform destroy and PendingDeletion" >&2
    exit 1
  fi
  exit "$rc"
}
...
aws kms verify --region "$AWS_REGION" \
  ...
  --signing-algorithm ML_DSA_SHAKE_256 \
  --query '{SignatureValid:SignatureValid,SigningAlgorithm:SigningAlgorithm}' \
...
jq -n \
  ...
  '{status:$status,timestamp:$timestamp,account_verified:true,region:$region,key_spec:$key_spec,kms_verify:true,openssl_verify:true}' \
